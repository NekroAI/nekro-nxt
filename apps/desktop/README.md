...
最后一条分发命令会同时执行最终 `appOutDir` 的 Server readiness 验证；只检查 staging、依赖文件存在或安装包摘要不能替代该验证。
