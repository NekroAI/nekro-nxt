...
工作模式默认打开上次频道。创造工作台由智能体或 Extension 的明确动作进入，不占据图标轨。工作轨迹是频道画布的一种视图，不是一级目的地。
...
工作模式的主对象是当前频道或当前智能体，不是智能体卡片墙。默认打开上次使用的 Channel；没有记录时打开第一个频道或第一个智能体工作台。对象列按智能体分组列出频道，运行状态写在智能体或频道条目上。“使用工具”“空闲”“已暂停”描述的是智能体 Runtime，不代表 Connection 或 Host 状态。
...
网页频道以后台用户视角排列：智能体在左、后台用户在右；外部频道以频道观察视角排列：平台成员在左，智能体和管理员机器人账号发言在右。消息使用中央全部可用宽度，单独文本块安全渲染 Markdown，连续 text 与 Mention 按原文位置内联。Composer 只在会话视图浮动于消息底部，常态从左到右保留信息图标、短发送模式、输入和靠右的发送动作；工作轨迹视图不显示 Composer。
...
| 发送 | 网页频道：把管理员输入作为入站消息交给绑定智能体。外部频道：以机器人账号发到该频道，并让智能体知道这是管理员从网页发出的 |

普通聊天消息永远不隐式等于停止。Composer 不放“能力”“创造状态”等脱离发送语境的按钮。

### 4.4 智能体工作台：发布新 Revision，而非覆盖现场

智能体工作台是单页滚动文档：人设与模型、已绑定频道、授权能力、已关联扩展。主对象是当前智能体的配置草稿。子智能体、网页搜索、文件工具、动态创造、开发 Shell、不受限文件访问分别授权；高风险能力只警示、不按频道强制封禁，开启创造不隐式获得 Shell 或宿主文件权限。
...
页面呈现扩展名称、当前版本、目标智能体、Contribution、持久验证证据、最近一次 Client 加载和启用状态，操作为“启用给智能体”或“停用扩展”。含 Client Contribution 的已启用版本可在扩展详情和智能体工作台渲染产品面板；单个 Client 失败被 Error Boundary 隔离，可“重新加载界面”，不会撤销 Host Tool 或 RPC。版本回退、删除和新建修改会话尚未开放。
...
**交互：** 连接 → 对象列「添加平台连接」→ 选择已安装平台 → 填写可选连接别名与平台账号 → 发现频道 → 在连接页选择智能体并完成绑定。已有可创建平台的账号详情提供「再添加一个账号」，直接进入该适配器配置表单。不必先跳到智能体管理页。收发测试留在连接详情的诊断区，不进入主路径。已创建的非系统托管连接可在连接详情保存或清除别名；系统托管 Web 连接不提供编辑入口。
