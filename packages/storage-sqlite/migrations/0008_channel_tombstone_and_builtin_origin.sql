...
CREATE TABLE `__new_episodes` (
	...
	CONSTRAINT "episodes_close_reason_ck" CHECK("__new_episodes"."close_reason" IS NULL OR "__new_episodes"."close_reason" IN ('manual', 'context-cleared', 'context-compacted', 'idle-timeout', 'incompatible-revision', 'incompatible-activation', 'incompatible-session-storage', 'unrecoverable-session', 'permission-revoked', 'binding-replaced', 'channel-deleted', 'stopped'))
);
...
ALTER TABLE `channels` ADD `auto_created_for_agent_id` text REFERENCES agent_definitions(id);--> statement-breakpoint
ALTER TABLE `channels` ADD `deleted_at` integer;