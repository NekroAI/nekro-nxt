...
需求在已授权智能体的频道里描述。创造页只展示该智能体正在运行的动态 Package、Client 预览审批和“保存为本地扩展”。系统不为创造另建智能体。保存不自动启用；启用在扩展页完成。
...
页面呈现扩展名称、当前版本、目标智能体和启用状态，操作为“启用给智能体”或“停用扩展”。版本回退、删除和新建修改会话尚未开放。
