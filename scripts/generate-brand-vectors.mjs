...
const productIllustration = (
  ...
  <rect width="480" height="320" rx="28" fill="#fffdf9"/>
  <circle cx="240" cy="154" r="104" fill="#f0f5fd" stroke="#bdcfec" stroke-width="2"/>
  ...
  <g fill="none" stroke="#172a45" stroke-width="7" stroke-linecap="round" stroke-linejoin="round">${body}</g>
</svg>`
...
const installerBackground = ({ channel, kind, width, height }) => {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img" aria-label="${title} ${kind}">
  ...
  ${width > 300 ? `<text x="36" y="56" fill="#172a45" font-family="Inter, ui-sans-serif, system-ui, sans-serif" font-size="26" font-weight="700">${title}</text><text x="330" y="386" text-anchor="middle" fill="#466394" font-family="Inter, ui-sans-serif, system-ui, sans-serif" font-size="14">拖动 NekroNXT 到 Applications 完成安装</text>` : ''}
  ${preview ? `<g transform="translate(${width * 0.68} ${height * 0.08})"><circle cx="0" cy="0" r="5" fill="#d0a66c"/><circle cx="18" cy="0" r="5" fill="#3fb1ea"/><circle cx="36" cy="0" r="5" fill="#d0a66c"/></g>` : ''}
</svg>`
}
