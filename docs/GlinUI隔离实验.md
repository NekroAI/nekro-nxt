...
48px Renderer 顶栏是无系统标题栏窗口的连续上盖，最外层窗口圆角由 macOS、Windows 和 Linux 宿主负责。品牌与实验标识直接置于连续顶栏，不使用底色、描边或内阴影。图标轨、对象列和主画布从顶栏下沿连续向下，主体左右贴合窗口边缘，顶部不重复收圆，只在窗口主体底部形成 18px 外轮廓。平台窗口控件安全区只作用于顶栏，不得给主体制造左右留白。分栏边界只占 1px，居中的 1px 视觉线与覆盖两侧的 9px 拖拽热区互相独立。控件、列表选择、面板与 Dialog 分别使用 10px、12px、16px 与 20px 半径；页面过渡层不裁剪表面投影，窄内容相对主画布内容区居中，避免相邻表面出现无意义的弧度和中心轴跳变。

用户、扩展和设置采用全宽桌面工作区。页头、搜索与筛选工具栏、表头和分页属于稳定外壳，数据表体、对象列表和详情正文分别使用独立滚动区；筛选、选择对象或切换设置分类时只更新必要内容。空态占满所属可用区域，透明插画与文字整体居中，不创建悬浮空态卡片。模型供应商与 DSH 扩展使用紧凑主从分栏，列表从顶部连续排列，详情独立滚动。
