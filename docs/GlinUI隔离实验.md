...
48px Renderer 顶栏是无系统标题栏窗口的连续上盖，最外层窗口圆角由 macOS、Windows 和 Linux 宿主负责。顶栏内的品牌与实验标识使用无描边、无内阴影的轻量玻璃底色分组；图标轨、对象列和主画布从顶栏下沿连续向下，主体左右贴合窗口边缘，顶部不重复收圆，只在窗口主体底部形成 18px 外轮廓。平台窗口控件安全区只作用于顶栏，不得给主体制造左右留白。控件、列表选择、面板与 Dialog 分别使用 10px、12px、16px 与 20px 半径，避免相邻表面出现无意义的弧度跳变。
