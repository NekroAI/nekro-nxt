...
网页频道以后台用户视角排列：智能体在左、后台用户在右；外部频道以频道观察视角排列：平台成员在左，智能体和管理员机器人账号发言在右。消息使用中央全部可用宽度，所有文本内容块安全渲染 Markdown。Composer 只在会话视图浮动于消息底部，常态从左到右保留信息图标、短发送模式、输入和靠右的发送动作；工作轨迹视图不显示 Composer。
