...
2. 一个智能体可以同时绑定网页、群聊和私聊等多个频道；每个频道只由一个智能体负责，并持续保留自己的实时场景与短期上下文；
...
- 一个智能体同一时间只保留一个有效 Channel Binding；切换到其他 Adapter 的 Channel 时保留旧绑定历史；
...
创造能力组合包括 DSH 动态 Cordis 工具，以及 NekroNxt Runtime 知识、智能体/Channel/Extension Inspect Provider、本地扩展保存和启用工具。智能体 Revision 当前独立记录子智能体、文件工具、网页搜索、动态创造、开发 Shell和不受限文件访问六项授权；扩大访问范围不会隐式开启文件工具或开发 Shell。
...
1. 文件工具、Bash、动态创造和不受限文件访问由用户在智能体管理中显式开启；文件工具默认关闭，并如实说明当前 DSH 只限制写入位置、读取仍受宿主进程权限控制；
