...
- `contracts`：跨持久与进程边界的不透明 ID、消息内容和运行时校验；
- `adapter-sdk`、`adapter-web`：平台能力、入站、物理投递和 Internal Web Adapter；
- `core`：智能体 Revision、Connection、Channel、Binding、Event 与 Asset Service；
- `channel-runtime`：Episode、Admission、Outbox、回执和当前频道历史契约；
- `client-migrations`：纯函数客户端状态迁移与宿主升级步骤协调；
