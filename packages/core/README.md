...
该包拥有 NekroNXT 的产品事实与不可变版本语义：智能体、Connection、Channel、Binding 和规范化 Channel Event。Connection 的 `alias` 是可选、trim 后最多 80 个字符的用户辨识名，不替换 Adapter 的平台身份。它不依赖具体聊天平台，不读取 DSH 私有存储，也不负责模型循环。
