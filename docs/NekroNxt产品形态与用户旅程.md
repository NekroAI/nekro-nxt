...
**交互：** 连接 → 对象列「添加平台连接」→ 选择已安装平台 → 填写可选连接别名与平台账号 → 发现频道 → 在连接页选择智能体并完成绑定。已有可创建平台的账号详情提供「再添加一个账号」，进入该适配器配置表单。收发测试留在连接详情的诊断区，不进入主路径。已创建的非系统托管连接可在连接详情保存或清除别名；系统托管 Web 连接不提供编辑入口。
...
**理想行为：** 每个实例的数据和运行完全独立；切换销毁旧产品页面并加载目标实例自己的页面；连接失败显示目标实例的可信错误页。

**交互：** 点击左下角“服务实例” → 选择已有实例，或填写地址、名称和管理密钥添加远程实例 → Desktop 固定服务器身份并建立设备会话 → 自动切换到目标工作区。
