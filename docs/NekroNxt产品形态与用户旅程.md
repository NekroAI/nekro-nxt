...
零智能体时画布是创建空态。对象列的 `+` 打开创建向导。暂停响应尚未作为主操作开放。

| 操作 | 前提 | 结果 |
|---|---|---|
| 创建智能体 | 用户可管理当前实例 | 打开智能体 Draft 向导 |
...
页面标题必须是当前 Channel“QQ 用户群”，副标题才说明“由小奈响应”。中央只出现该群的消息；其他频道只存在于工作模式对象列。
...
| 发送 | 仅网页频道：把管理员输入作为入站消息交给绑定智能体。外部平台频道不能从网页冒充平台发言 |
...
**交互：** 连接 → 添加连接 → 选择已安装平台（例如 QQ）→ 填写平台账号 → 测试接收/发送 → 在连接页选择智能体并完成绑定。不必先跳到智能体管理页。
