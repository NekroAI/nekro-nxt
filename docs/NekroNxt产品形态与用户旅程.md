...
动态创造、开发 Shell 和完整文件访问默认关闭。外部 QQ/Discord Channel 以后再绑定，不把首次上手变成部署向导。
...
页面主对象是“基于 r12 的配置草稿”。普通能力、动态创造、开发 Shell、完整文件访问分别授权；开启创造不隐式获得 Shell 或宿主完整文件权限。

| 操作 | 真实后果 |
|---|---|
| 保存为新配置 | 生成 r13；新 Session 使用 r13；已有 Session 保持 r12 |
| 保存并在空闲后切换 | 生成 r13，并让活动 Session 在 Tool 结束后的安全间隙滚动 |
...
页面主对象是 Extension Draft“网页摘要助手”，目标智能体“小奈”同时是对话、动态运行与激活上下文。普通用户通过对话向已启用创造能力的小奈描述需求，结构化界面展示计划、权限、变更和验证证据。系统不会为创造过程另建一个“创造助手”智能体。
...
| 保存为本地插件 | 写入本地扩展目录并生成不可变 Revision，不自动授权智能体|
...
“新建修改会话”会创建新 Draft 并进入创造工作台，不直接改写 r3。切换默认 Revision 只影响未来 Activation 或新 Session；各智能体仍可固定在不同 Revision。只要存在活动 Activation，“删除本地插件”就不可执行，并明确说明前置条件。
...
→ 发布 r13
→ 新 Session 自动使用 r13
→ 活动 Session 保持 r12，或被安排在安全间隙滚动到 r13
```
...
**交互：** 连接 → 添加 QQ → 登录账号 → 选择群 → 选择小奈 → 设置触发规则 → 测试接收/发送 → 完成绑定。
...
**交互：** 管理小奈 → 开启动态创造 → 进入创造工作台 → 描述需求 → 查看计划与权限 → 动态运行 → 补充测试/要求修改 → 保存为本地插件 → 启用给小奈。
